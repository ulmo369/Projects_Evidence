...
def get_model(normalization_tensor, column_count):
  ...
  model = Sequential([
        #normalization_layer,
        LSTM(50, return_sequences=True, input_shape=(1,column_count)),
...
def trainTFModel(prepared_data_path, models_path):
    ...
    for i, file_name in enumerate(glob.glob(csv_folder_path)):
        ...
        if i == 0:
            ...
            #arr_normalization_sample = normalization_sample.values[:,np.newaxis,:]
    ...
    train_data_majority = train_data_majority.sample(frac = train_data_minority.shape[0]/train_data_majority.shape[0], random_state = 2)
    ...
    x_train_tensor = np.reshape(x_train_tensor, (x_train_tensor.shape[0], 1, x_train_tensor.shape[1]))
    normalization_tensor = tf.convert_to_tensor(normalization_sample)
    normalization_tensor = np.reshape(normalization_tensor, (normalization_tensor.shape[0], 1, normalization_tensor.shape[1]))
    
    model = get_model(normalization_tensor, len(df_x_train.columns))
...
def testTFModel(prepared_data_path, models_path):
    ...
    test_data_filtered = test_data.loc[test_data['col5_float'] > 1] # Filtro de datos a predecir (> 17500)
    ...
    x_test_tensor = np.reshape(x_test_tensor, (x_test_tensor.shape[0], 1, x_test_tensor.shape[1]))
    model = load_model(os.path.join(models_path, 'saved_model_tf.keras'))

    print('Testing start')
    predicted_indexes = test_data_df[test_data_df['col5_float'] > 1].index # (> 17500)
